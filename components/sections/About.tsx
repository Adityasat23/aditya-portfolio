    ...
    <section id="about" className="mesh-gradient relative overflow-hidden py-20">
      <div className="section-container">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-16 text-center">
          ...
          <p className="mx-auto max-w-2xl text-gray-400">Bringing Quality with Quantity at the same time</p>
        </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="flex flex-col justify-center">
            ...
            <SpotlightCard className="rounded-2xl border border-white/20 bg-black/40 p-6 backdrop-blur-xl">
              ...
              <div className="space-y-3">
                <a href={`mailto:${personalInfo.email}`} className="block text-sm text-gray-300 transition-colors hover:text-blue-400">📧 {personalInfo.email}</a>
                <a href={`tel:${personalInfo.phone}`} className="block text-sm text-gray-300 transition-colors hover:text-blue-400">📱 {personalInfo.phone}</a>
